...
var App = React.createClass({
  ...
  render() {
    return (
      <div>
        <div
          className="area-for-global-focus-loss"
        ...
        <Select.Select
          ...
          displayingChildRenderer={(child, isExpanded, isFocused) => {
            return (
              <div
                style={{ border: `4px solid ${isFocused ? '#f88' : '#555'}` }}
              >
                {child}
              </div>
            );
        ...
        >
          {exampleConstants.data.map((d, i) => {
            return (
              <Select.Option value={d.value} label={d.label} key={d.value}>
                {(hover, active, selected) => {
                  console.log('this function is being called');
                  return (
                    <div
                      className={`${exampleConstants.SELECT_CLASS_PREFIX}-${i}${
                        selected ? '-selected' : ''
                      }`}
                    >
                      {d.label +
